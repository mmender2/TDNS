...
contract Domains is ERC721URIStorage, ERC721Enumerable {
    ...
    constructor(string memory _tld)
        payable
        ERC721("Tfuel Name Service", "TNS")
    {
        owner = payable(msg.sender);
        tld = _tld;
        console.log("%s deployed", _tld);
    }
    function isAvail(string calldata name) public view returns(string memory) {
        string memory isAvailable = '';
    ...
    function register(string calldata name) public payable {
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the tfuel name service", "image": "data:image/svg+xml;base64,',
    ...
    function getAllTokensOfOwner(address owner_) external view returns(uint[] memory tokenIds){
        uint bal = balanceOf(owner_);
        tokenIds = new uint256[](bal);
        for (uint i = 0; i < bal; ++i){
            tokenIds[i] = tokenOfOwnerByIndex(owner_, i);
        }
    }
    function getAllTokenURIs(address owner_) external view returns(string[] memory uris){
        uint bal = balanceOf(owner_);
        uris = new string[](bal);
        for (uint i = 0; i < bal; ++i){
            uris[i] = tokenURI(tokenOfOwnerByIndex(owner_, i));
        }
    }
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return .5 * 10**17; // 5 TFUEL = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.5 TFUEL cause the faucets don't give a lot
        } else if (len == 4) {
    ...
    function withdraw() public onlyOwner {
        ...
        require(success, "Failed to withdraw tfuel");
    }
